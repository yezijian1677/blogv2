...
Axis.prototype = {
	...
	update: function (newOptions, redraw) {
		...
		newOptions = chart.options[this.xOrY + 'Axis'][this.options.article] = merge(this.userOptions, newOptions);
	...
	remove: function (redraw) {
		...
		chart.options[key].splice(this.options.article, 1);
		...
		each(chartSeries, function (series) {
			...
			if (isString(linkedTo)) {
				if (linkedTo === ':previous') {
					linkedTo = chart.series[series.article - 1];
				} else {
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		stableSort(chartSeries, function (a, b) {
			return pick(a.options.article, a._i) - pick(b.options.article, a._i);
		});
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.article) ||
							(seriesOptions[AXIS] !== UNDEFINED && seriesOptions[AXIS] === axisOptions.id) ||
							(seriesOptions[AXIS] === UNDEFINED && axisOptions.article === 0)) {
	...
	setStackedPoints: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			stack = stacks[key][x];
			stack.points[series.article] = [stack.cum || 0];
			...
			stack.points[series.article].push(stack.cum);
	...
	setPercentStacks: function () {
		...
		each([stackKey, '-' + stackKey], function (key) {
			...
			while (i--) {
				...
				pointExtremes = stack && stack.points[series.article];
				if (pointExtremes) {
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			if (stacking && series.visible && stack && stack[xValue]) {
				...
				stackValues = pointStack.points[series.article];
